...
function renderCSS(props, currentBreakpoint) {
  ...
  return Object
    ...
    .map((prop) => {
      ...
      const currentCssPropValue = isCssPropValueAnObject ? cssPropValue[currentBreakpoint] : undefined;

      if (currentBreakpoint === 'xs' && !isCssPropValueAnObject) {
        return renderCSSValue(cssPropName, cssPropValue);
      }

      if (currentCssPropValue !== undefined) {
...
export function Icon({
  ...
  return (
    <Box styleSheet={finalStyleSheet}>
      <FontAwesomeIcon
